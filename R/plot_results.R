...
plot_res <- function(mod1,
  ...
  pred_res$model_label <- ifelse(mod1$EIV, mod1$model,paste0("eiv-",mod1$model))
  ...
  if (!is.null(mod2)) {
    ...
    pred_res1$model_label <- ifelse(mod1$EIV, mod1$model,paste0("eiv-",mod1$model))
    pred_res2$model_label <- ifelse(mod2$EIV, mod2$model,paste0("eiv-",mod2$model))
